...
##### 纯函数
提到纯粹性，不提到另一个重要的概念-纯函数。 纯函数其实就是数学中的函数。它有一个重要的特征就是给定输入，输出是一定的。前面这句话可以从两方面理解，一是给定输入，输出一定，一对一。 另一点是没有副作用（side effect），副作用指的是函数经过运算不会对外界产生影响，当然也不依赖外界。 其实上面的两点是一点，没有副作用正是给定输入输出一定的前提，只是理解角度不同而已。

##### 单一职责
纯粹性除了上面的特征之外，还有一个重要特征是单一职责。 即一个功能函数只完成原子性功能，不要让函数做非常多的事情，保证其功能的纯粹。
